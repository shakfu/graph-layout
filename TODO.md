...
- [x] **Sugiyama coordinate assignment is index-based** (Done): replaced the independent per-layer centering with Brandes-Köpf horizontal coordinate assignment (`hierarchical/_brandes_koepf.py`) -- four vertical-alignment runs (align up/down x median left/right) with type-1 conflict marking, balanced per vertex. Each vertex aligns with the median of its neighbours, so edges (especially long-edge dummy chains) are straightened. The shift-class compaction from the paper was replaced with a longest-path block compaction that guarantees the within-layer ordering / min-separation invariant by construction (verified 0 violations over thousands of random graphs; a final clamp is a defensive safety net). Regression tests: `tests/test_brandes_koepf.py` (invariant + straightening) and a Sugiyama-level assertion that a long edge's bends are vertically collinear.
