...
class TestSpectralLayout:
    ...
    def test_path_graph_ordering(self):
        ...
        for i in range(len(result_nodes) - 1):
            dist = math.sqrt(
                (result_nodes[i].x - result_nodes[i + 1].x) ** 2 +
                (result_nodes[i].y - result_nodes[i + 1].y) ** 2
            )
    ...
    def test_normalized_vs_unnormalized(self):
        ...
        layout1.nodes(nodes).links(links).size([800, 600])
        layout1.normalized(True).start()

        nodes2 = [{} for _ in range(5)]
        links2 = [{'source': i, 'target': i + 1} for i in range(4)]

        layout2 = SpectralLayout()
        layout2.nodes(nodes2).links(links2).size([800, 600])
        layout2.normalized(False).start()
    ...
    def test_community_separation(self):
        ...
        clique1_x = sum(result_nodes[i].x for i in range(4)) / 4
        clique1_y = sum(result_nodes[i].y for i in range(4)) / 4
        clique2_x = sum(result_nodes[i].x for i in range(4, 8)) / 4
        clique2_y = sum(result_nodes[i].y for i in range(4, 8)) / 4
    ...
    def test_single_node(self):
        ...
        assert abs(result_nodes[0].x - 400) < 1.0
        assert abs(result_nodes[0].y - 300) < 1.0
